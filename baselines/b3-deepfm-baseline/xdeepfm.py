...
"""第三步：调用fit()函数训练模型"""
model.fit(train_model_input, train[target].values, batch_size=512, epochs=30, validation_data=[vaild_model_input, vaild[target].values], verbose=1, model_cache_path="E:\competition\看山杯\models\\xdeepfm111.model")
...
result.to_csv(path + 'submit111.txt', sep='\t', index=False)   # 注意提交的时候请把表头去掉
